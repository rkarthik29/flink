...
import com.cloudera.example.flink.udf.MyStringJoinFunction;

/**
 * Hello world!
 *
 */
public class FlinkExample_Kafka_Sink {
    public static void main(String[] args) {
        try {
            ...
            DataStream<String> violationData = geoEvent.join(speedEvent).where(keySelector, TypeInformation.of(Integer.class)).equalTo(keySelector, TypeInformation.of(Integer.class))
                    .window(TumblingEventTimeWindows.of(Time.seconds(1))).apply(new MyStringJoinFunction());

            FlinkKafkaProducer<String> producer = new FlinkKafkaProducer<String>("violations_karthik", new SimpleStringSchema(), properties);
